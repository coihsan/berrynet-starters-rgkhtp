...
export default function Footer(){
    ...
    return(
        <footer className='containers-center'>
            ...
            <ul className={'accordionContent flex flex-col gap-2 pb-[1rem] '}>
                {ourproducts.map((ourproducts)=>(
                    <li className={'hover:translate-x-[5px] text-[14px] transition-transform ease-in-out cursor-pointer'}><Link className='' href={ourproducts.path}>{ourproducts.title}</Link></li>
                ))}
            </ul>
            </AccordionItem>
            ...
            <ul className={'accordionContent flex flex-col gap-2 pb-[1rem] '}>
                {company.map((company)=>(
                    <li className={'hover:translate-x-[5px] text-[14px] transition-transform ease-in-out cursor-pointer'}><Link className='' href={company.path}>{company.title}</Link></li>
                ))}
            </ul>
            </AccordionItem>
            ...
            <ul className={'accordionContent flex flex-col gap-2 pb-[1rem] '}>
                {helpsupport.map((helpsupport)=>(
                    <li className={'hover:translate-x-[5px] text-[14px] transition-transform ease-in-out cursor-pointer'}><Link className='' href={helpsupport.path}>{helpsupport.title}</Link></li>
                ))}
            </ul>
            </AccordionItem>
            </Accordion>
