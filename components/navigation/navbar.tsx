    ...
    return (
        <nav className="fixed max-[768px]:p-[1rem] w-[100%] backdrop-blur-xl bg-white/10 h-auto">
            <div className="lg:p-4 items-center justify-between text-sm lg:flex max-w-screen-xl mx-auto">
                    ...
                    {navitems.map((menu) =>(
                        <Link style={menu.theme} href={menu.path} className='text-white hover:text-indigo-100 px-5 py-2 hover:bg-indigo-600 rounded-full transition-all'> {menu.title} {menu.icon}</Link>
                    ))}
                    </div>
            </div>
        </nav>
